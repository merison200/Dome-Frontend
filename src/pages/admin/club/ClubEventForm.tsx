...
import { Calendar, Clock, Plus, X, Music, Users, Tag, ArrowLeft } from 'lucide-react';

interface Props {
  mode: 'create' | 'edit';
}

const ClubEventForm: React.FC<Props> = ({ mode }) => {
  const { id } = useParams();
  ...
  const [newHypeman, setNewHypeman] = useState('');
  ...
  const addDj = () => {
    if (newDj.trim()) {
      setFormData((prev) => ({ ...prev, dj: [...prev.dj, newDj.trim()] }));
      setNewDj('');
    }
  };
  ...
  const removeDj = (index: number) => {
    setFormData((prev) => ({ 
      ...prev, 
      dj: prev.dj.filter((_, i) => i !== index) 
    }));
  };

  const addHypeman = () => {
    if (newHypeman.trim()) {
      setFormData((prev) => ({ ...prev, hypeman: [...prev.hypeman, newHypeman.trim()] }));
      setNewHypeman('');
    }
  };

  const removeHypeman = (index: number) => {
    setFormData((prev) => ({ 
      ...prev, 
      hypeman: prev.hypeman.filter((_, i) => i !== index) 
    }));
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          <button
            onClick={() => navigate('/admin/club/events')}
          ...
          <h1 className="text-2xl font-bold text-gray-900">
            {isEdit ? 'Edit Event' : 'Create Club Event'}
          </h1>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"
                placeholder="Experience the ultimate electronic music journey..."
              />
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2">
                <input
                  type="text"
                  value={newHypeman}
                  onChange={(e) => setNewHypeman(e.target.value)}
                  placeholder="Add hypeman name"
                  className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"
                  onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addHypeman())}
                />
                <button
                  type="button"
                  onClick={addHypeman}
              ...
              <div className="flex flex-wrap gap-2">
                {formData.hypeman.map((hypeman, index) => (
                  <span
                    key={index}
                    className="flex items-center gap-1 px-3 py-1 bg-red-50 text-red-700 rounded"
                  >
