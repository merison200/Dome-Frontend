...
const LoungeEventDetails: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const [event, setEvent] = useState<Lounge | null>(null);
    ...
    const fetchEvent = async () => {
      try {
        if (id) {
          ...
          setEvent(response.data as Lounge);
        }
      } catch (err) {
        console.error(err);
...
export default LoungeEventDetails;